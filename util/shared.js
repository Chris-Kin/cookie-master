import SuccessImg from "data-base64:~assets/success.svg"
import ErrorImg from "data-base64:~assets/error.svg"
import WarnImg from "data-base64:~assets/warn.svg"
...
export const setCookiesAsync = async function(cookie, isInCurrentDoamin) {
    ...
    return new Promise(function (res, rej) {
        chrome.cookies.set(adaptedCookie).then(newCookie => {
            if (!newCookie) {
...
export function getCookies() {
    return chrome.tabs.query({ currentWindow: true, active: true }).then(tabs => {
        if (!tabs?.[0]) {
            return Promise.reject('can not get tabs');
        }
